...
; (hash/c string 1) (hash/c symbol value) (values string (hash/c string number))
; read all of the resumes
(define (read-resumes stop-words options); (values resume-file-name bag-of-words)
  (define (path->bow path fraction-retained)
    (define port (open-input-file path #:mode 'text))
    (define str (port->string port #:close? #t))
    (string->bow str fraction-retained))
  (define fraction-resume-retained (hash-ref options 'fraction-resume-retained))
  ...
  (for/hash ((fso fsos))
    ...
    (when (file-exists? path-to-file)
      (values (path->string fso)
              (hash-difference (path->bow path-to-file fraction-resume-retained)
                               stop-words)))))
